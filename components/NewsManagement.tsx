...
export default function NewsManagement({ initialArticles }: { initialArticles: ArticleRow[] }) {
  ...
  return <>
    ...
    <div className="admin-empty" style={{ maxWidth: 'none', overflowX: 'auto' }}>
      {filtered.length === 0 ? <p>No matching articles found.</p> : <table className="admin-table"><thead><tr><th><input aria-label="Select all visible articles" type="checkbox" checked={filtered.length > 0 && filtered.every((article) => selected.includes(article.id))} onChange={(event) => setSelected(event.target.checked ? filtered.map((article) => article.id) : [])} /></th><th>Title</th><th>Category</th><th>Author</th><th>Status</th><th>Published</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{filtered.map((article) => <tr key={article.id}><td><input aria-label={`Select ${article.title}`} type="checkbox" checked={selected.includes(article.id)} onChange={() => toggle(article.id)} /></td><td><strong>{article.title}</strong><br /><small>{article.slug}</small></td><td>{article.category?.name || 'Uncategorized'}</td><td>{article.author.name}</td><td>{article.status}</td><td>{article.publishedAt ? new Intl.DateTimeFormat('en-GB', { dateStyle: 'medium' }).format(new Date(article.publishedAt)) : '—'}</td><td>{new Intl.DateTimeFormat('en-GB', { dateStyle: 'medium' }).format(new Date(article.updatedAt))}</td><td><a className="admin-action" href={`/admin/news/${article.id}`}>EDIT</a><a className="admin-action" href={`/news/${article.slug}`} target="_blank" rel="noreferrer">VIEW</a></td></tr>)}</tbody></table>}
    </div>
  </>;
}
