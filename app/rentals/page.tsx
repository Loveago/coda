...
export default async function RentalsPage() {
  const [vehicles, site] = await Promise.all([
    db.vehicle.findMany({
      where: { availability: 'AVAILABLE', dailyRate: { not: null } },
      ...
      <main>
        <section className="page-hero">
          <div className="container">
            <p className="kicker">CAR RENTALS</p>
            <h1>Move on your terms.</h1>
            <p>Browse available rental vehicles and send your preferred dates. Our team will confirm availability and next steps.</p>
          </div>
        </section>
          ...
          <div className="vehicle-grid">
            {vehicles.length ? (
              vehicles.map((vehicle) => (
                <article className="vehicle-card" key={vehicle.id}>
                  {vehicle.images[0] ? (
                    ...
                    <div className="vehicle-placeholder"><Car size={30} /> MRT</div>
                  )}
                  <div className="vehicle-card-body">
                    <span className="vehicle-category">{vehicle.category} · AVAILABLE</span>
                    <h2>{vehicle.make} {vehicle.model}</h2>
                    <p>{vehicle.dailyRate ? `GHS ${vehicle.dailyRate.toLocaleString()} / day` : 'Rate on request'}</p>
                    ...
                    <Link href={`/contact?vehicle=${encodeURIComponent(`${vehicle.make} ${vehicle.model}`)}&type=rental`} className="btn btn-primary">
                      REQUEST RENTAL <ArrowRight size={14} />
                    </Link>
                  </div>
                </article>
              ))
              ...
              <div className="empty-state">
                <h2>Rental vehicles are being prepared.</h2>
                <p>Contact Mr Truth Agency for current rental availability and rates.</p>
                <Link href="/contact?type=rental" className="btn btn-primary">
