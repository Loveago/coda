...
export default function AdminLogin() {
  ...
  return <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: 'linear-gradient(140deg,#061b49,#0a54c9)', padding: 20 }}><form onSubmit={submit} style={{ width: 'min(100%,420px)', background: '#fff', borderRadius: 16, padding: 32, display: 'grid', gap: 16, boxShadow: '0 20px 60px #00143d55' }}><div className="brand"><div className="brand-mark">◉</div><div><div className="brand-name">GACODA</div><small className="brand-sub">ADMIN PANEL</small></div></div><h1 style={{ fontFamily: 'Barlow Condensed', fontSize: 36, margin: '16px 0 0' }}>Administrator sign in</h1><p style={{ color: 'var(--muted)', fontSize: 13, margin: 0 }}>Use your approved GACODA CMS account.</p><input name="email" type="email" required placeholder="Email address" style={field}/><input name="password" type="password" required placeholder="Password" style={field}/><button className="btn btn-primary" disabled={busy}>{busy ? 'SIGNING IN...' : 'SIGN IN'}</button>{error && <p role="alert" style={{ color: '#c62828', fontSize: 12 }}>{error}</p>}</form></main>;
}
